...
PERSON_HOG = create_person_detector()


def detect_people(frame_bgr: np.ndarray, scale: float = 0.5) -> Tuple[int, List[List[int]]]:
    small = cv2.resize(frame_bgr, (0, 0), fx=scale, fy=scale)
    rects, weights = PERSON_HOG.detectMultiScale(small, winStride=(8, 8), padding=(8, 8), scale=1.05)
    boxes: List[List[int]] = []
    for (x, y, w, h), _ in zip(rects, weights):
...
def stream_frames(source: str = "0", motion: bool = False, threshold: int = 25, min_area: int = 800) -> Iterable[bytes]:
    """Yield MJPEG frames. If `motion` is True, perform lightweight frame-diff motion detection
    and log motion events (with a short cooldown) when motion is detected.
    """
    cap = open_capture(source)
    counter = 0
    prev_gray = None
    ...
    while True:
        ...
        # Motion detection (lightweight) — operate on a small grayscale thumbnail
        motion_score = 0.0
        if motion:
            try:
                ...
                if prev_gray is not None:
                    ...
                    motion_score = float(sum(cv2.contourArea(c) for c in contours))
                    now_t = time.time()
                    if motion_score >= float(min_area) and (now_t - last_event_time) > cooldown_secs:
                        # Log event + save image via existing pipeline
                        result = log_image_pipeline(frame.copy(), source_type="motion_stream", device_id=f"camera:{source}", note=f"motion_score={round(motion_score,2)}, threshold={threshold}, min_area={min_area}")
                        motion_event = {
                            "event_id": f"evt_{uuid.uuid4().hex[:10]}",
                            ...
                            "severity": "WARNING",
                            "explanation": "Motion detected in live stream.",
                            "action_hint": "Review captured image in dashboard.",
                        }
                        append_csv(EVENT_CSV, EVENT_FIELDS, motion_event)
                        last_event_time = now_t
                        cv2.rectangle(frame, (0, 40), (frame.shape[1], 80), (0, 0, 255), -1)
                        cv2.putText(frame, f"MOTION! score={int(motion_score)}", (10, 66), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)
            except Exception:
                # Don't let motion errors break the stream
                motion_score = 0.0

        # Overlay status bar and frame info
        cv2.rectangle(frame, (0, 0), (frame.shape[1], 32), (255, 255, 255), -1)
        status_text = f"{source_label} | source={source} | frame={counter}"
        if motion:
            status_text += f" | motion_on threshold={threshold} min_area={min_area}"
        cv2.putText(frame, status_text, (10, 22), cv2.FONT_HERSHEY_SIMPLEX, 0.58, (0, 0, 0), 2)

        jpg = frame_to_jpeg_bytes(frame)
        yield b"--frame\r\nContent-Type: image/jpeg\r\n\r\n" + jpg + b"\r\n"
        counter += 1
        prev_gray = small_gray if motion else None
        time.sleep(0.08)
...
def people_count(source: str = Query("0", description="0 for laptop camera, or IP camera/video URL"), scale: float = Query(0.5, gt=0, le=1.0, description="Downscale factor for faster people detection")) -> Dict[str, Any]:
    frame, source_type = read_one_frame(source)
    count, boxes = detect_people(frame, scale=scale)
    result = log_image_pipeline(frame, source_type=source_type, device_id=f"camera:{source}", note=f"people_count={count}")
    event_type = "PEOPLE_DETECTED" if count > 0 else "NO_PEOPLE_DETECTED"
    event_row = {
        "event_id": f"evt_{uuid.uuid4().hex[:10]}",
        "image_id": result["image_id"],
        "timestamp": now_iso(),
        ...
        "score": count,
        "severity": "NORMAL",
        "explanation": "Detected people in the camera frame." if count > 0 else "No people detected in the camera frame.",
        "action_hint": "Use the captured image for further review." if count > 0 else "Continue monitoring the camera stream.",
    }
    append_csv(EVENT_CSV, EVENT_FIELDS, event_row)
    result["people_count"] = count
    result["people_boxes"] = boxes
    result["people_event"] = event_row
    return result
...
@app.get("/video_feed")
def video_feed(source: str = Query("0"), motion: bool = Query(False, description="Enable motion detection while streaming"), threshold: int = Query(25, ge=1, le=255), min_area: int = Query(800, ge=10, le=50000)) -> StreamingResponse:
    """MJPEG video feed. Set `motion=true` to enable live motion detection and event logging."""
    return StreamingResponse(stream_frames(source, motion=motion, threshold=threshold, min_area=min_area), media_type="multipart/x-mixed-replace; boundary=frame")
